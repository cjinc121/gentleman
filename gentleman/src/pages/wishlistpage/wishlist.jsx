...
return <div>
  <div className="page-header">Wishlist({userState.wishlist.length})</div>
    ...
    <div className="wishlist-container">
      {
        userState.wishlist.length===0&&<div className="page-header" > Empty<GiNotebook/></div>
      }
      {
userState.wishlist.map((item)=>{
  ...
  return <div className="card-container-item">
  <div className="card-img">
    <img src={item.photoUrl} />
    <div className="tag-image-right cart-icon"onClick={()=>userDispatch({type:"REMOVE_FROM_WISHLIST",payload:item})}>
      <MdCancel/>
    </div>
  </div>
